...
field fields[] = {
	{ FIELD_NAME,      "NAME",      10, compare_name,      print_ident		},
...
void do_header(void)
{
	...
	for(i = 0; i < NUM_FIELDS; i++) {
		...
		print("%*s%s", fields[i].default_width, fields[i].header, isSort);
	}
	print("\n");
}
...
int which_identifier(char *identifier) {
	...
	for(int i = 1; i <= IDENTIFIER_MAXOPTS; i++) {
		identCompare = identifierOptions[i];

		inputsize = strnlen(identifier, 10);
		comparesize = strnlen(identCompare, 10);
		
		if (inputsize == comparesize)
			size = comparesize;
			
			if (inputsize < comparesize)
				size = inputsize;
			
			if (strncmp(identCompare, identifier, size) == 0)
				found = i;
	}
